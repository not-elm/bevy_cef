...
pub fn notify_screen_info_changed(&self, webview: &Entity) {
    if let Some(browser) = self.browsers.get(webview) {
        // `notify_screen_info_changed` itself only updates Chromium's cached
        // screen metrics and causes CEF to re-query `GetScreenInfo` /
        // `GetRootScreenRect` / `GetViewRect` — it does not explicitly promise
        // an immediate `OnPaint` (per `cef_browser.h:710-730`). To guarantee
        // the user sees the new DPR reflected in the next frame, we also
        // invalidate the view, which matches the cefclient OSR convention.
...
**Call order at the call-site:** `set_dpr` first, then `notify_screen_info_changed`. Reversing the order would cause CEF to re-query with the stale DPR. (`invalidate` inside `notify_screen_info_changed` fires *after* the CEF cache update, so we observe the new DPR on the forced repaint.)
...
4. `notify_screen_info_changed` updates Chromium's cached screen metrics but does not explicitly promise an immediate `OnPaint` (`cef_browser.h:710-730`). The spec therefore pairs it with `host.invalidate(PaintElementType::VIEW)` to guarantee the user sees the new DPR on the next frame, matching the cefclient OSR convention.
5. `cef::PaintElementType::VIEW` variant name to be verified at implementation time (cef-rs may expose as `PaintElementType::View` or `PaintElementType::VIEW` depending on binding generation).
